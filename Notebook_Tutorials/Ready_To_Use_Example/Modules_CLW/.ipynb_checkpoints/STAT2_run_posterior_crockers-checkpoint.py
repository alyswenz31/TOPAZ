...
def run_save_crocker(args):
    ...
    DATA_COLS, true_FRAME_LIST, pred_FRAME_LIST, PROX_VEC, iSample = args
...
#    print(max(filt_df.x))
    ...
    SAVE_PATH = os.path.join(save_dir,'crocker_angles.npy')
    np.save(SAVE_PATH,crocker)

def run_posterior_crockers(num_post_samples):
    ...
    list_tuples = []
    for iSample in range(num_post_samples):
        list_tuples.append((DATA_COLS, true_FRAME_LIST, pred_FRAME_LIST, PROX_VEC, iSample))

    for i in range(len(list_tuples)):
        run_save_crocker(list_tuples[i])
