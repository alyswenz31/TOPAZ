...
from Scripts.Alignment import flores_align
...
#             x[x==0] = 0 #25*1.16162489196
            x[x<0] = 25*1.16162489196
        if 'right' in self.BCs:
#             vx[x>25*1.16162489196] = -np.abs(vx[x>25*1.16162489196])
#             x[x==25*1.16162489196] = 0
            x[x>25*1.16162489196] = 0
        if 'top' in self.BCs:
#             vy[y<0] = np.abs(vy[y<0])
#             y[y==0] = 0 #25*0.88504753673
            y[y<0] = 25*0.88504753673
        if 'bottom' in self.BCs:
#             vy[y>25*0.88504753673] = -np.abs(vy[y>25*0.88504753673])
#             y[y==25*0.88504753673] = 0
            y[y>25*0.88504753673] = 0
#         print('diff_align')
        output = np.hstack((vx,vy,dvxdt,dvydt))
        return output
    
    def diff_align(self,t,Z):
        '''
        Gets derivatives of position and velocity of particles according to the
        D'Orsogna model of soft-core particle interactions.
        ...
        # Include an alignment term for sensitivity to neighbors with a 
        # sensitivity parameter omega. We are going to assume a neighborhood to be ~10% of
        # the area which represents a radial distance of 5 for a 30 x 30 grid.
        pos = np.vstack((x, y)).T
        vel = np.hstack((vx[:, np.newaxis], vy[:, np.newaxis]))

        scaled_align = flores_align(pos, vel, N_radius=5, weight=self.w, dist_bool=False)

        # Now update dxdt and dydt with alignment
        dvxdt += np.squeeze(scaled_align[:,0])
        dvydt += np.squeeze(scaled_align[:,1])
...
#         print('diff_align')
    ...
    def ode_rk4(self,ic_vec,t0,tf,
        ...
        '''
        simu = [ic_vec]
        r = ode(self.diff_align).set_integrator('dopri5',atol=10**(-3))
    ...
    def position_gif(self,figure_dir,time_vec):
        ...
        for j,(t,vec) in enumerate(zip(time_vec,self.simulation_results)):
            ...
            plt.xlim(0,25*1.16162489196)
            plt.xlabel("x")
            plt.ylim(0,25*0.88504753673)
            plt.ylabel("y")
#             plt.axis('off')
#             plt.xticks([], [])
#             plt.yticks([], [])
    ...
    def results_to_df(self,time_vec):
        ...
        for j,(vec,t) in enumerate(zip(self.simulation_results,time_vec)):
            ...
            df = pd.concat([df, frame],ignore_index=True) # type: ignore
