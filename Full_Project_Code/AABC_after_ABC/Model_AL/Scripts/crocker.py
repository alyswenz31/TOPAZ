...
def plot_crocker_highres_split(crocker,prox_vec,frame_list,crocker1,prox_vec1,frame_list1,CLWvals,
                 save_path=None):
    fig,(ax1,ax2) = plt.subplots(1,2,sharex=True,sharey=True,figsize=(12,8),dpi=400)
    fig.suptitle('C: '+ str(round(CLWvals[0],3)) + ', L: ' + str(round(CLWvals[1],3)) + ', W: ' + str(round(CLWvals[2],3)), fontsize=30)       #ADDED
    bcs0 = ax1.contourf(*np.meshgrid(range(1,crocker.shape[0]+1),
                                     prox_vec,indexing='ij'),
                        crocker[:,:,0],(1,50,100,150,200,250))
