...
PARAM_UPPER = np.array([1.3, 3.0, 0.1], dtype=float)
PARAM_SCALE = PARAM_UPPER - PARAM_LOWER
ABC_SAMPLE_SIZE = 23_100
...
def scale_theta(theta: np.ndarray) -> np.ndarray:
    """Map C, L, and W from their prior ranges to the unit cube."""
    values = np.asarray(theta, dtype=float)
    if values.shape[-1] != 3:
        raise ValueError(f"Expected final parameter dimension 3, got {values.shape}")
    return (values - PARAM_LOWER) / PARAM_SCALE
