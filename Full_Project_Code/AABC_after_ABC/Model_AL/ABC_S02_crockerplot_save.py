...
def run_save_crocker(args):
    ...
    DF_PATH = './Simulated_Grid/ODE_Align/sample_'+str(NUM_SAMPLE)+'/run_'+str(iSample+1)+'/df.pkl'
    unscale_num = 25
...
#    print(max(filt_df.x))
    ...
    if 'vx' in DATA_COLS:
        SAVE_PATH = './Simulated_Grid/ODE_Align/sample_'+str(NUM_SAMPLE)+'/run_'+str(iSample+1)+'/crocker_velocities.npy'
    if 'angle' in DATA_COLS:
        SAVE_PATH = './Simulated_Grid/ODE_Align/sample_'+str(NUM_SAMPLE)+'/run_'+str(iSample+1)+'/crocker_angles.npy'
...
NUM_SAMPLE = 20800 # Update to match chosen grid size from ABC_S01_samples.py
