...
#        print(iSample)
    ...
    for iSample in range(chosen_NUM_SAMPLE_AABC):
#        print(iSample)

        # update indexing number to begin after chosen_NUM_SAMPLE has ended 
        index = iSample + chosen_NUM_SAMPLE + 1

        pars_path = './Simulated_Grid/ODE_Align/sample_aabc_'+str(chosen_NUM_SAMPLE_AABC)+'/run_'+str(iSample+1)+'/theta_star.npy'
        pred_path = './Simulated_Grid/ODE_Align/sample_aabc_'+str(chosen_NUM_SAMPLE_AABC)+'/run_'+str(iSample+1)+'/crocker_angles.npy'
      
        if os.path.isfile(pred_path):
            par_values = np.load(pars_path, allow_pickle=True)
            pred_crocker = np.load(pred_path, allow_pickle=True)

            # change pars from length 3 to length 6 where CLW are 3,4,5
            ...
            losses[str(index)] = {}
            losses[str(index)]['sampled_pars'] = par_values
            losses[str(index)]['loss'] = loss
...
max_NUM_SAMPLE = 20800
chosen_NUM_SAMPLE = 20800
chosen_NUM_SAMPLE_AABC = 5000
