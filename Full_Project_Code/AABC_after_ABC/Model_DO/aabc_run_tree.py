...
def init_worker():
    ...
    if not np.all(np.isfinite(Theta_ref)):
        bad = np.argwhere(~np.isfinite(Theta_ref))
        raise ValueError(
            f"all_params.npy contains NaN or inf values at {bad[:10].tolist()}"
        )

    scaled_ref = scale_theta(Theta_ref)
    if not np.all(np.isfinite(scaled_ref)):
        bad = np.argwhere(~np.isfinite(scaled_ref))
        raise ValueError(
            "Scaled reference parameters contain NaN or inf values at "
            f"{bad[:10].tolist()}"
        )

    Theta_ref_tree = cKDTree(scaled_ref)
